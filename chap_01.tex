...
\begin{equation}
 f(x,y,p)=d(x,y)\dfrac{\partial u}{\partial x}+e(x,y)\dfrac{\partial u}{\partial y}
	  r(x,y)u+s(x,y)
\end{equation}
则方程~\ref{eq:01_ejt_2}~为二阶线性方程.\par
...
\end{equation*}
代入式~\ref{eq:01_f_jfs}~,在第一项中令$\lambda=-\mu$,即
\begin{equation*}
...
\end{cases}
\end{equation}\par
下面,我们来介绍二段龙格库塔公式的推导过程.已知龙格库塔公式的一般形式为方程~\ref{eq:01_lg_1},得到二段龙格库塔公式
的格式为
...
假设$y_n=y(x_n)$,并将上式代入式~\ref{eq:01_lg_n}~,化简得
...
\begin{equation}\label{eq:01_lg_tl}
 ...
 \end{split}
\end{equation}
逐项比较式~\ref{eq:01_lg_b}~和式~\ref{eq:01_lg_tl}.为了使$y(x_{n+1})-y_{n+1}=O(h^3)$,则有
